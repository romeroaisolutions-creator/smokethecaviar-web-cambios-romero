...
import { Wallet, ShieldCheck, Gem, Sparkles, Fingerprint, Loader2 } from 'lucide-react';
...
const Products = () => {
  ...
  const handleBuy = async (product: typeof productsData[0]) => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo('.product-card',
        { y: 50, opacity: 0, scale: 0.95 },
        {
          ...
          scrollTrigger: { trigger: '.carousel-container', start: 'top 75%' }
        }
      );
  ...
  return (
    <section id="productos" ref={containerRef} className="py-32 bg-black-carbon relative border-b border-white/5 scroll-mt-20">
      <div className="max-w-7xl mx-auto px-8 relative z-10">
        ...
        {/* Product Carousel (Grid for now) */}
        <div className="carousel-container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-32">
            ...
            <div key={prod.id} className="product-card group relative bg-black-deep rounded-2xl p-6 border border-white/5 hover:border-gold-metallic/30 overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:glow-gold">
              ...
              <div className="relative w-full aspect-square bg-[#0c0c0c] rounded-xl mb-6 overflow-hidden flex items-center justify-center mix-blend-lighten">
                ...
                <img 
                  src={productImg} 
                  alt={prod.name} 
                  className="w-[80%] h-[80%] object-cover object-center group-hover:scale-110 transition-transform duration-700 z-0" 
                />
              </div>
            ...
            <div className="relative group/btn">
              <button
                onClick={async () => {
                  ...
                  try {
                    await redirectToCheckout(productsData[0].name, productsData[0].price);
                  } catch {
                    alert('Hubo un error al iniciar el pago. Intenta de nuevo.');
                  } finally {
