...
import { ShieldCheck, Gem, Sparkles, Fingerprint, Loader2, Shirt, ArrowUpRight } from 'lucide-react';
import merchImg from '../assets/brand_logo.png';
import { useLang } from '../context/LanguageContext';

type ProductType = 'tee' | 'hoodie' | 'cap' | 'access';

type Product = {
  id: number; name: string; displayPrice: string; price: number;
  category: string; type: ProductType;
};
...
const Products = () => {
  ...
  const visibleProducts = useMemo(
    () => (activeFilter === 'all' ? merchProducts : merchProducts.filter((prod) => prod.type === activeFilter)),
    [activeFilter],
  );
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo('.badge-item', { y: 30, opacity: 0 }, {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo('.product-row', { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, stagger: 0.05, ease: 'power3.out' });
    }, containerRef);
    return () => ctx.revert();
  }, [activeFilter]);

  return (
    <section id="productos" ref={containerRef} className="py-20 md:py-28 bg-black-carbon relative border-b border-white/5 scroll-mt-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 relative z-10">
        <div className="mb-10 md:mb-14 text-center max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 mb-4">
            ...
            <span className="text-gold-soft uppercase tracking-[0.3em] text-sm font-semibold">{p.label}</span>
          </div>
          <h2 className="text-4xl md:text-5xl mb-5 font-playfair text-glow">{p.h2}</h2>
          <p className="text-gray-smoke text-lg leading-relaxed font-light">{p.p}</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 mb-10 md:mb-12">
          {p.filters.map((f) => {
            ...
            return (
              <button
                key={f.key}
                onClick={() => setActiveFilter(f.key as 'all' | ProductType)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-2 mb-14 max-w-5xl mx-auto">
          {visibleProducts.map((prod) => (
            <div key={prod.id} className="product-row group flex items-center justify-between gap-6 py-5 border-b border-white/5 hover:border-gold-metallic/30 transition-colors">
              <div className="flex items-center gap-5 min-w-0">
                <div className="hidden sm:flex w-12 h-12 rounded-lg bg-black-deep border border-white/5 items-center justify-center overflow-hidden flex-shrink-0">
                  <img src={merchImg} alt="" className="w-8 h-8 object-contain opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500" />
                </div>
                <div className="min-w-0">
                  <div className="text-[10px] uppercase tracking-[0.25em] text-gold-soft/80 mb-1 truncate">
                    {p.categories[prod.category] ?? prod.category}
                  </div>
                  <div className="font-playfair text-lg md:text-xl text-white-ivory truncate">{prod.name}</div>
                </div>
              </div>

              <div className="flex items-center gap-5 flex-shrink-0">
                <span className="text-sm font-light text-gray-smoke font-mono tracking-wider hidden sm:block">{prod.displayPrice}</span>
                <button
                ...
                >
                  {loadingId === prod.id ? (
                    <><Loader2 className="w-3.5 h-3.5 animate-spin" /> {p.processing}</>
                  ) : (
                    <>{p.buy}<ArrowUpRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" /></>
                  )}
                </button>
              </div>
            </div>
        ...
        <div className="badges-container grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 text-center border-t border-white/5 pt-14">
          {[
            { icon: Gem,         data: p.badge1 },
            { icon: Fingerprint, data: p.badge2 },
            { icon: Sparkles,    data: p.badge3 },
          ].map(({ icon: Icon, data }) => (
            <div key={data.title} className="badge-item flex flex-col items-center">
              ...
              <h4 className="text-base font-medium mb-2 text-white-ivory">{data.title}</h4>
              <p className="text-gray-smoke text-sm font-light">{data.p}</p>
            </div>
          ))}
        </div>
      </div>
